<script lang="ts">
	...
	(async () => {
		if (docId) {
			GlobalComment = (await import(`$lib/data/global_comments/${annot[docId]?.globCommId}.svelte`))
				.default;
		}
	})();
</script>
...
{#if metadata[docId]}
	<div class="px-40 pb-10">
		...
		<div class={['relative my-5 border-4 p-5', isExpanded ? 'pb-20' : 'pb-0']}>
			<!-- Box -->
			<div class={[isExpanded ? 'h-auto' : 'max-h-40 overflow-hidden']}>
				<!-- Global Comment -->
				<h5 class="my-4 h5"><strong>Kommentar</strong></h5>
				<div data-dom="global-comment">
					<GlobalComment />
				</div>

				<!-- Metadata Table -->
				<h5 class="my-4 h5"><strong>Metadaten</strong></h5>
				<div data-dom="metadata-table">
				...
				<!-- Keywords -->
				<div>
					<h5 class="my-4 h5"><strong>Schlagwörter</strong></h5>
					<div data-dom="keywordlist-header" class="flex flex-wrap gap-2">
						{#each Object.keys(dictReg) as regType}
							...
							{#each regKeys ? Object.values(regKeys) : [] as regKey}
								<a
									...
									data-type="keyword"
									data-keywordtype={dictReg[regType].singular}
...
<style>
	...
	[data-dom='global-comment'] {
		:global(p) {
			@apply my-2;
		}
	}

	[data-dom='keywordlist-header'] {
		:global([data-type='keyword']::before) {
			@apply rounded-xl px-1 font-bold;
			content: '↗';
		}
		:global([data-keywordtype='person']) {
			@apply rounded-xl bg-red-100 px-2 font-bold;
		}
		:global([data-keywordtype='place']) {
			@apply rounded-xl bg-green-100 px-2 font-bold;
		}
		:global([data-keywordtype='event']) {
		...
		:global([data-keywordtype='keyword']) {
