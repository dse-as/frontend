...
def transform_photos(document):
    ...
    2. Remove '[SLA-...]' pattern from the 'name' field
    3. Rename 'id_sla' to 'sla_id_full' and split into 'sla_id_coll' / 'sla_id_img'
    4. Transform 'stamp' key: rename to 'stamped' (bool/null)
    5. Transform 'signatures' key: rename to 'signed' (bool/null), ignoring array values
    """
    ...
    for photo_key, photo_data in photos.items():
        ...
        # (2) Remove '[SLA-...]' from name using regex
        if "name" in photo_data:
            # Removes [SLA-anything] including brackets, then strips extra whitespace
            photo_data["name"] = re.sub(r'\[SLA-[^\]]*\]', '', photo_data["name"]).strip()
            # Optional: Clean up double spaces resulting from removal
            photo_data["name"] = re.sub(r'\s{2,}', ' ', photo_data["name"])
        ...
        if "stamp" in metadata:
            ...
            if stamp_val is None:
                ...
                # Fallback for unexpected values, keeping it as string or setting to None
                # Here we default to None to maintain type consistency (bool/null)
                metadata["stamped"] = None
        ...
        if "signatures" in metadata:
            ...
            if sig_val is None:
                ...
                # Ignore array content, just check the logical intent based on your description
                # Assuming the list contains exactly one item like ["signiert"] or ["nicht signiert"]
                ...
                # If it's not a list or null, handle gracefully
                metadata["signed"] = None
