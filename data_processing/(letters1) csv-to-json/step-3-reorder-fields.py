...
- Delete top-level "type", rename metadata "type" to "types"
...
def strip_trailing_date(title: str | None) -> str | None:
    ...
    # Pattern: comma followed by a date (with optional range) at the very end
...
def clean_iiif_url(url: str | None) -> str | None:
    ...
    # Check if it ends with the specific postfix
...
def reorder_letter(letter_obj: dict) -> dict:
    ...
    # Filter out None values if any (though clean_iiif_url returns None only if input is None)
    ...
    transformed = {
        ...
        "metadata": metadata,
        "citedDocuments": {
            "smallforms": letter_obj.get("citedDocuments", {}).get("smallforms", []),
            "longforms": letter_obj.get("citedDocuments", {}).get("longforms", []),
            "photos": letter_obj.get("citedDocuments", {}).get("photos", []),
        },
        "entities_temp": {
            "travel": letter_obj.get("entities_temp", {}).get("travel"),
        },
        "editorialNotes": {
...
def transform_file(input_path: str, output_path: str):
    ...
    for letter_key, letter_obj in letters.items():
        raw_urls = letter_obj.get("manuscript", {}).get("iiif_urls", [])
        # Count how many URLs will be cleaned in this letter
...
if __name__ == "__main__":
    ...
    output_file = "letters_output_with_pages_cleaned(step3).json"
